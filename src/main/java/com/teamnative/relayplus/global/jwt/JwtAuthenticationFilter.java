 ...
 *
 * [수정사항]
 * - authenticate() 메서드에 try-catch 블록 추가
 * - getEmail() / getRole() 호출 시 JwtException 발생 시 TOKEN_STATUS_ATTRIBUTE를 INVALID로 설정
 * - 이를 통해 500 에러 대신 401 Unauthorized가 반환되도록 개선
 */
@Component
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /**
     * 토큰에서 이메일과 권한을 추출하여 SecurityContext에 등록합니다.
     * [수정사항] JwtException 처리 추가 - 파싱 중 예외 발생 시 401 Unauthorized로 처리
     */
    private void authenticate(HttpServletRequest request, String token) {
        try {
            ...
            // JWT 파싱/검증 중 예외 발생 시, 토큰이 유효하지 않음으로 처리
            // JwtAuthenticationEntryPoint가 이를 읽어 401 Unauthorized 응답
            request.setAttribute(TOKEN_STATUS_ATTRIBUTE, JwtTokenProvider.TokenStatus.INVALID);
        }
    }
